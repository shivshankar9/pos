  ...
  useEffect(() => {
    const checkSignInStatusAndSubscription = async () => {
      ...
      if (isSignedIn && user && user.primaryEmailAddress?.emailAddress) {
        ...
        const isSubscribed = checkSubscription(email);

        if (isSubscribed) {
          router.push("/dashboard"); // Redirect to dashboard if subscribed
        } else {
          router.push("/subscribe"); // Redirect to subscribe page if not subscribed
        }
      } else if (!isSignedIn) {
