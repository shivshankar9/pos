...
const generateResume = (template, userDetails, jobs) => {
  ...
  const skills = userDetails.skills ? userDetails.skills.split(",").join(", ") : "N/A";
  resume = resume.replace("{{skills}}", skills);

  const jobDescriptions =
    jobs.length > 0
      ? jobs.map((job) => `${job.role} at ${job.company}\n${job.location}`).join("\n\n")
      : "N/A";
      ...
      <div className="max-w-4xl w-full">
        <form className="bg-white p-8 rounded-lg shadow-xl space-y-6">
          ...
          <textarea
            ...
            onChange={(e) =>
              setUserDetails({ ...userDetails, jobDescriptions: e.target.value })
            }
            className="w-full p-3 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
            required
          ></textarea>
        ...
        {resume && (
          <div className="mt-8 bg-white p-8 rounded-lg shadow-lg">
            <h3 className="text-2xl font-bold mb-4 text-gray-800">Generated Resume</h3>
